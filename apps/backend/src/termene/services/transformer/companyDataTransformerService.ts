...
export class CompanyDataTransformerService {
  ...
  transformCompanyData(cui: string, data: CompanyTermeneDataset) {
    ...
    if (registrationNumber.length) {
      ...
      companyInfo.registrationNumber.metadata.trustworthiness.source =
        sourceUrl;
    }

    companyInfo.headquarters =
      this.locationDataTransformerService.transformHeadquartersData(data) ??
      null;
    companyInfo.locations =
      this.locationDataTransformerService.transformBranchesData(data);
    ...
    if (data.headerInfo?.statut_fiscal) {
      companyInfo.status.fiscal.value =
    ...
    if (data.headerInfo?.stare_firma) {
      ...
      if (mfinante) {
        ...
        companyInfo.active.ministryOfFinance.metadata.trustworthiness.source =
          sourceUrl;
      }

      if (recom) {
        companyInfo.active.tradeRegister.value = recom.curenta.functiune;
        companyInfo.active.tradeRegister.metadata.trustworthiness.source =
          sourceUrl;
      }
    }
    return companyInfo;
  ...
  private setActivityCodes(data: CompanyTermeneDataset, sourceUrl: string) {
    ...
    if (mainCAEN) {
      ...
      activityCodes.push(
        this.createCustomField(sourceUrl, String(mainCAEN.cod), mainCAEN.label),
      );
    }
  ...
  private setContactDetails(
    { contactDetails }: CompanyTermeneDataset,
    sourceUrl: string,
  ) {
    ...
    contactDetails?.Web?.split(',').forEach((website) => {
      ...
      if (cleanWebsite.length) {
        contactDetailsList.push(
          this.createCustomField(sourceUrl, 'Website', cleanWebsite),
        );
      }
    });
  ...
  private setBalanceSheets = (data: CompanyTermeneDataset, sourceUrl: string) =>
    data?.balanceSheet?.balanceSheet.map((balanceSheetInfo) => {
      const balanceSheet: BalanceSheet = {
        ...
        houseAndAccountsSeizedByBanks:
          balanceSheetInfo.casa_si_conturi_la_banci ?? 0,
  ...
  private setCompanyStatus = (data: CompanyTermeneDataset, sourceUrl: string) =>
    this.createCustomField(
      sourceUrl,
      'Status',
      data.headerInfo?.stare_firma.mfinante.curenta.functiune
        ? 'Activa'
        : 'Inactiva',
    );

  private setCompanyFicalStatus = (
    data: CompanyTermeneDataset,
    sourceUrl: string,
  ) => [
    this.createCustomField(
    ...
    this.createCustomField(
      sourceUrl,
      'Statut TVA',
      data.headerInfo?.statut_tva.curent.label ?? '',
    ),
  ];
