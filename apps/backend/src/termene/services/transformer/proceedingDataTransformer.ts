...
import { AUTHOR } from '../../../../../backend-services/apps/osint-termene/src/constants';
import {
  TermeneInvolvedEntity,
  TermeneProceeding,
} from '../../schema/courtFiles';
...
export class ProceedingDataTransformer {
  ...
  private async resolveInvolvedEntities(
    entitiesInfo: TermeneInvolvedEntity[],
    sourceUrl: string,
  ) {
    ...
    for await (const entityInfo of entitiesInfo) {
      switch (entityInfo.tip) {
        case 'PF': {
          const involvedPerson = await this.getInvolvedPerson(
            entityInfo,
            sourceUrl,
          );
        ...
        case 'PJ': {
          const involvedCompany = await this.getInvolvedCompany(
            entityInfo,
            sourceUrl,
          );
  ...
  private async getInvolvedPerson(
    involvedEntity: TermeneInvolvedEntity,
    sourceUrl: string,
  ) {
  ...
  private async getInvolvedCompany(
    involvedEntity: TermeneInvolvedEntity,
    sourceUrl: string,
  ) {
    const companyId = await this.getCompany(
      involvedEntity.denumire,
      String(involvedEntity.cui),
    );
