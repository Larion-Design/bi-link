...
export class AssociateScraperService {
  ...
  private async isPersonAssociateOfCompany(
    page: Page,
    associateUrl: string,
    cui: string,
  ) {
  ...
  private traverseSearchResults = async (
    ...
    Promise.all(
      tableRows.map(async (tableRow) => {
        ...
        const personId = await tableRow.$eval(
          "form input[name='company_id']",
