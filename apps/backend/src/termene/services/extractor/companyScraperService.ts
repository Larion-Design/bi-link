...
import {
  companyHeaderInfoSchema,
  companyProfileSchema,
} from '../../schema/generalInfo';
...
export class CompanyScraperService {
  ...
  async extractCompanyData(cui: string) {
    const dataset = await this.termeneAuthService.authenticatedPage(
      async (page) => {
        ...
        await page.waitForResponse(async (response) => {
          ...
          if (success) {
            ...
            if (url.includes('CAEN.php')) {
              ...
              dataset.contactDetails = contactDetailsSchema.parse(
                await response.json(),
              );
            } else if (url.includes('DataV2.php?type=header')) {
              dataset.headerInfo = companyHeaderInfoSchema.parse(
                await response.json(),
              );
            } else if (url.includes('DataV2.php?type=companyProfile')) {
              dataset.profileInfo = companyProfileSchema.parse(
                await response.json(),
              );
            } else if (url.includes('balanceSheet.php')) {
              dataset.balanceSheet = balanceSheetSchema.parse(
                await response.json(),
              );
            } else if (url.includes('courtFilesV2.php')) {
              dataset.courtCases = courtFilesSchema.parse(
                await response.json(),
              );
            } else if (url.includes('associatesAdministrators.php')) {
              dataset.associates = associatesSchema.parse(
                await response.json(),
              );
            }
          }
            ...
            if (elem) {
              ...
              await page.waitForResponse(async (response) => {
                ...
                if (success && response.url().includes('searchCompany.php')) {
                  const data = searchCompaniesByNameSchema.parse(
                    await response.json(),
                  );
                  data.forEach(({ nume, cui }) =>
                    companies.push({ cui: String(cui), name: nume }),
                  );
                }
                return success;
              });
      ...
      async (context) =>
        this.browserService.handlePage(
          ...
          async (page) => {
            ...
            const { name, registrationNumber, headquarters } =
              await page.$$eval(
