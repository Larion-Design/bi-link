...
export class BrowserService {
  ...
  constructor(
    ...
    this.chromiumInstanceUrl = configService.getOrThrow<string>(
      'CHROMIUM_INSTANCE_URL',
    );
  }
  ...
  async handlePage<T = void>(
    ...
    try {
      ...
      if (options?.blockResources?.length) {
        ...
        await this.blockRedundantResources(
          page,
          this.getAllResourceTypesExceptHTML(),
        );
      }
      ...
      if (options?.persistentCookiesKey) {
        ...
        if (cookies?.length) {
          await this.persistentCookiesService.persistCookies(
            options.persistentCookiesKey,
            cookies,
          );
        }
  ...
  private async blockRedundantResources(
    page: Page,
