import { companyRegistrationNumberRequest } from '@frontend/graphql/companies/queries/companyRegistrationNumberExists'
...
export const validateRegistrationNumberLength = (registrationNumber: string) => {
  if (registrationNumber.length < 3) {
    return 'Numărul de inregistrare trebuie sa contina minim trei caractere.'
  }
}

export const validateRegistrationNumberFormat = (registrationNumber: string) => {
...
const validateUniqueRegistrationNumber = async (registrationNumber: string, companyId?: string) => {
  const { data, error } = await companyRegistrationNumberRequest(registrationNumber, companyId)
